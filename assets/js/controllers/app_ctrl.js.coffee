walletApp.controller "AppCtrl", ($scope, Wallet, $state, $rootScope, $location, $cookieStore, $timeout, $modal, $window, $translate) ->
  ...
  $scope.$on('$stateChangeSuccess', (event, toState, toParams, fromState, fromParams) ->
    if toState.name != "login.show" && toState.name != "login" && toState.name != "register" && toState.name != "recover" && toState.name != "open" && toState.name != "verify-email" && toState.name != "verify-email-with-guid" && $scope.status.isLoggedIn == false
      $state.go("login.show")
